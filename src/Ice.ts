...
export class Ice extends Ally {
  private specialAttack: number = 50; // 特殊攻撃

  constructor(name: string) {
    super(name, 120, 120, 40);
  ...
  doAttack(enemies: any[]): boolean {
    ...
    if (randomValue1 === 5) {
      //強力な技の発動
      console.log(`『${this.getName()}』のぜったいれいど!`);
      console.log("いちげき ひっさつ!");
      console.log(`【 ${enemy.getName()} 】をやっつけた!`);
      enemy.tookDamage(enemy.getHitPoint());
    } else if (randomValue2 === 3) {
      //ノーマル技の発動
      console.log(`『${this.getName()}』のれいとうビーム!`);
